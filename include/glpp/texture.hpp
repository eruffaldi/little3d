...
public:
    ...
    void initdepth(GLSize size, GLuint type)
    {
        release();
        format_ = GL_DEPTH_COMPONENT;
        channels_ = 1;
        ...
        glGenTextures(1, &resource_);
        glBindTexture(GL_TEXTURE_2D, resource_);
        glTexImage2D(GL_TEXTURE_2D, 0, type == GL_FLOAT ? GL_DEPTH_COMPONENT32F : GL_DEPTH_COMPONENT24, size_.width, size_.height,
                         0, format_, type_, NULL);
...
//        std::cout << "loaded texture " << width << "x" << height << " resulting " << w << "x" << h << std::endl;
    ...
    bool getDataRaw(std::vector<uint32_t> & data) const
    {
        data.resize(width() * height() * channels_);
        glPixelStorei(GL_PACK_ALIGNMENT, 1);
        glBindTexture(GL_TEXTURE_2D, resource_);
        glGetTexImage(GL_TEXTURE_2D, 0, format_, GL_UNSIGNED_INT, &data[0]);
        glBindTexture(GL_TEXTURE_2D, 0);
        return true;
    }

    bool getDataRaw(std::vector<uint16_t> & data) const
    {
        data.resize(width() * height() * channels_);
        glPixelStorei(GL_PACK_ALIGNMENT, 1);
        glBindTexture(GL_TEXTURE_2D, resource_);
        glGetTexImage(GL_TEXTURE_2D, 0, format_, GL_UNSIGNED_SHORT, &data[0]);
        glBindTexture(GL_TEXTURE_2D, 0);
        return true;
    }

    bool getDataRaw(std::vector<float> & data) const
    {
        if(type_ != GL_FLOAT)
            return false;
        glPixelStorei(GL_PACK_ALIGNMENT, 1);
        ...
        glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_BUFFER_SIZE, &bs);
        std::cerr << "getDataRaw " << width() << " " << height() << " " << channels_ << " " << textureWidth << " " << textureHeight << " " << bs<< std::endl;
        glGetTexImage(GL_TEXTURE_2D, 0, format_, type_, &data[0]);
        glBindTexture(GL_TEXTURE_2D, 0);
        return true;
    }
        ...
        switch(type_)
        {
            ...
            case GL_UNSIGNED_INT:
            {
               ...
               if(channels_ == 1)
               {
                ...
                if(!getDataRaw(buffer))
                    return false;
                lodepng_encode_file(filename.c_str(), (uint8_t *)&buffer[0],
                                    size_.width, size_.height, LCT_GREY, 16);
                return true;
               }
                    else
                return false;                
            ...
            case GL_FLOAT:
            {
                std::cerr << "cannot save float image " << std::endl;
                //std::ofstream onf(filename.c_str(),std::ios::binary);
                //onf.write((char*)&buffer[0],buffer.size());
                return false;
            }
...
{
public:
    void init(GLSize size, GLuint type) { Texture::initdepth(size,type); }
};
